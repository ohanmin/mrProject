...
public class WordCount {
    ...
    public static void main(String[] args) throws Exception{
        //파라ㅣ터는 분석할 파일(폴더)과 분석 결과가 저장될 파일(폴더)로 2개를 받음
        if(args.length != 2){
        ...
        //멥리듀스 잡이 시작되는 main함수가 존재하는 파일 설정
        job.setJarByClass(WordCount.class);
        //멥리듀스 잡 이름 설정, 리소스 매니저 등 맵리듀스 실행 결과 및 로그 확인할 때 편리함
        job.setJobName("Word Count");
        //분석할 폴더(파일) -- 첮번째 파라미터
        FileInputFormat.setInputPaths(job, new Path(args[0]));
        //분석 결과가 저장되는 폴더(파일) --두번째 파라미터
        FileOutputFormat.setOutputPath(job, new Path(args[1]));
        //맵리듀스의 앱 역할을 수행하는 Maper 자바 파일 설정
        job.setMapperClass(WordCountMapper.class);
        //멥리듀스의 리듀스 역할을 수행하는 Reducer 자바 파일 설정
